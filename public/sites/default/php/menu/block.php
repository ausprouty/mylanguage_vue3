<?php
function block_format($title, $content){
...
function block_select_aboriginal(){
 return;
...
function block_select_australia(){
	return null;
...
function block_select_browser(){
	...
	$output = $space . l($ethnic_name, 
			'intro/'. $hl_id,
			 array('attributes' => array('class' => array($class))));
	db_set_active('default');
	return $output;
}	
function block_select_common(){
  $half = 10;
  $count = 0;
  $output = '';
...
function block_select_ethnic ($format = 'laptop'){
  ...
  switch ($block_key) {  
    ...
    case 'ask_options':
		...
		foreach($results as $data){
		    $block['content'] .= l($data->ethnic_name , '/ask/' . $data->hl_id ). ' - '. $data->name . '<br><br>' . "\n";
		}  
		break;
	case 'listen_options':
		$block['content']= '';
		...
		foreach ($results as $data){
			if (!$i) {
				$block['content'] .= mylanguage_t_ethnic('Listen to the Bible') . '<br>';
				$i = '1';
			}  
			$block['content'] .= '&nbsp;&nbsp;&nbsp;&nbsp;'. l($data->volume_name, 'listen_online/' . $hl_id . '/New Testament/'. $data->dam_id) . '<br>'. "\n";
		}
		$block['content'] .= '<br>';
		db_set_active('hl');
		...
		foreach($results as $data){
			...
			$block['content'] .= '&nbsp;&nbsp;&nbsp;&nbsp;' . l('Part 1', 'listen_online/' . $hl_id . '/'. $title .'/1') . '<br>'. "\n";
			$block['content'] .= '&nbsp;&nbsp;&nbsp;&nbsp;' . l('Part 2', 'listen_online/' . $hl_id . '/'. $title .'/2') . '<br>'. "\n";
		}
		break;
	case 'other_options':
		$block['subject']  = mylanguage_t_ethnic('Other Videos');
		...
		break;
	case('5_fish'):
		$block['subject']  = mylanguage_t_ethnic('5 Fish');
		...
		$block['content'] .= '<div style = "text-align:center; padding-top:25px;"><b>'. mylanguage_t_ethnic('FREE  Android App') . '</b></a></div>'; 
		$block['content'] .= mylanguage_t_ethnic('Listen to the message of God spoken in over 5,000 languages and dialects. Download free Bible stories, basic Bible teaching, Scripture, Christian songs and testimonies. Sharing it with others.');
		$block['content'] .= '</div>';
		break;
	case('bible_is'):
		$block['subject']  = mylanguage_t_ethnic('Listen or Read');
		...
		$block['content'] .= '<div style = "text-align:center; padding-top:25px;"><b>'. mylanguage_t_ethnic('FREE  Android App') . '</b></a></div>'; 
		$block['content'] .= mylanguage_t_ethnic('READ the Bible, LISTEN to the Bible, and SEE the Bible as never before with anyone in the world.');
		$block['content'] .= '  ';
		$block['content'] .= mylanguage_t_ethnic('Bible.is offers mobile access to more languages and Bible translations than any other Bible App, and includes dramatized Bible Audio and The JESUS Film Project.');
		break;
	case('jesus_media'):
		$block['subject']  = mylanguage_t_ethnic('JESUS Film Media');
		$block['content']  = '<image  align = "center" width = "90%" src = "'. mylanguage_url_file() .'apps/JESUSfilm.png"><br>';
		$block['content'] .= '<div style = "text-align:center; padding-top:25px;"><b>'. mylanguage_t_ethnic('FREE  App') . '</b></div>'; 
		$block['content'] .= mylanguage_t_ethnic('Access films in over 1,100 languages! Featuring the films JESUS, Magdalena, Story of Jesus for Children in full length and segments; the series Following Jesus and Walking with Jesus and catalog of short films like My Last Day.');
	...
	case('free_injil'): // only shown where Vance wants
		...
		if ( mylanguage_vance($hl_id)) {
			$block['subject']  = mylanguage_t_ethnic('FREE HOLY INJEEL');
			$block['content'] = '<img  align = "center" width = "90%" src = "'. mylanguage_url_file() .'apps/injil.jpg"><br>';
			$block['content'] .= l(mylanguage_t_ethnic('Ask for your FREE HOLY INJEEL today') , 'injeel') .'<br>';
			$block['content'] .= '<p style="color:green; font-size:0.7em">'. mylanguage_t_ethnic('Offer valid in USA and Canada') . '</p>';
    }
		break;
	case('you_version'):
		$block['subject']  = mylanguage_t_ethnic('God\'s Word');
		...
		$block['content'] .= '<div style = "text-align:center; padding-top:25px;"><b>'. mylanguage_t_ethnic('FREE Mobile App') . '</b></a></div>'; 
		$block['content'] .= mylanguage_t_ethnic('Tens of millions of people are using this to make God\'s Word a part of their daily lives. Available in more than 100 languages worldwide, all on your mobile device');
		break;
	case('dbs'):
		global $language;
		if (isset($_SESSION['mylanguage_chinese'])){
		...
		$block['subject']  = mylanguage_t_ethnic('Discussions');
		db_set_active('hl_online');
		...
		$block['content']= '';
		foreach($results as $data){
			$block['content'] .= l( $data->session . '. ' . $data->reference, 
